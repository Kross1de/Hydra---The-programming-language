...
class ArrayLiteral:
    ...
    def index(self, value, start=None, end=None):
        start = start or 0
        end = end or len(self.elements)
        
        try:
            return self.elements[start:end].index(value) + start
        except ValueError:
...
class IndexCall:
    def __init__(self, array, value, start=None, end=None):
        self.array = array
        self.value = value
        
        # Normalize start and end to primitive values or None
        def normalize_index(index):
            if index is None:
                return None
            
            # If it's already a Num node, get its value
            if isinstance(index, Num):
                return Num(Token('NUMBER', str(int(index.value)), 0, 0))
            
            # If it's a primitive value, create a Num node
            try:
                # Ensure it's an integer
                int_value = int(index)
                return Num(Token('NUMBER', str(int_value), 0, 0))
            except (TypeError, ValueError):
                # If conversion fails, return None
                return None
        
        self.start = normalize_index(start)
        self.end = normalize_index(end)

    def __repr__(self):
        return f"IndexCall({self.array}, {self.value}, {self.start}, {self.end})"
...
class CountCall:
    ...
    def __repr__(self):
        return f"CountCall({self.array}, {self.value})"
...
class Parser:
    ...
    def eat(self, token_type):
        ...
        raise Exception(f"Unexpected token {current_token}. Expected {token_type}")
    ...
    def if_statement(self):
        self.eat('IF')
        if self.current_token() is not None and self.current_token().type == 'LPAR':
            self.eat('LPAR')
    ...
    def for_statement(self):
        ...
        self.eat('SEMI')
        update = None
        if self.current_token() is not None and self.current_token().type != 'RPAR':
    ...
    def primary(self):
        ...
        if current.type == 'NUMBER':
            ...
            return Num(token)
        elif current.type == 'TRUE' or current.type == 'FALSE':
            token = self.eat(current.type)
            return Bool(token)
            ...
            if self.current_token().type == 'DOT':
                ...
                if method_name.value == 'index':
                    ...
                    self.eat('RPAR')
                    return IndexCall(node, value, start, end)
                
                elif method_name.value == 'count':
                    self.eat('LPAR')
                    value = self.expr()
                    self.eat('RPAR')
                    return CountCall(node, value)
            
            return node
        elif current.type == 'LPAR':
            self.eat('LPAR')
            if self.current_token() is not None and self.current_token().type == 'RPAR':
                self.eat('RPAR')
                return Unit()
                ...
                self.eat('RPAR')
                return node
        elif current.type == 'LSQUARE':
            return self.array_literal()
        else:
            raise Exception(f"Неожиданный токен: {current}")
...
class Interpreter:
    def __init__(self, tree_or_code):
        if isinstance(tree_or_code, str):
            lexer = Lexer(tree_or_code)
            tokens = lexer.tokenize()
            parser = Parser(tokens)
            self.tree = parser.parse()
        else:
            self.tree = tree_or_code
    ...
    def _safe_convert_to_index(self, index_obj):
        if index_obj is None:
            return None
        
        if isinstance(index_obj, Num):
            try:
                return int(index_obj.value)
            except (TypeError, ValueError):
                return None
        ...
        if hasattr(index_obj, '__index__'):
            try:
                return index_obj.__index__()
            except Exception:
                return None
    ...
    def visit_IndexCall(self, node):
        ...
        try:
            start_index = self._safe_convert_to_index(start)
            end_index = self._safe_convert_to_index(end)
            
            if start_index is not None and not isinstance(start_index, int):
                raise TypeError(f"Start index must be an integer, got {type(start_index)}")
            
            if end_index is not None and not isinstance(end_index, int):
                raise TypeError(f"End index must be an integer, got {type(end_index)}")
            
            if start_index is not None and end_index is not None:
                return array.index(value, start_index, end_index)
            elif start_index is not None:
                return array.index(value, start_index)
            else:
                return array.index(value)
        
        except TypeError as e:
            print(f"Index conversion error: {e}")
            print(f"Original Start: {start}, Converted Start: {start_index}")
            print(f"Original End: {end}, Converted End: {end_index}")
            raise
        
        except Exception as e:
            print(f"Unexpected error in index method: {e}")
            print(f"Array: {array}")
            print(f"Value: {value}")
            print(f"Start: {start}, Converted Start: {start_index}")
            print(f"End: {end}, Converted End: {end_index}")
            raise

    def visit_CountCall(self, node):
        array = self.visit(node.array)
        value = self.visit(node.value)
